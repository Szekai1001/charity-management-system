...
<div class="container py-4">
    ...
    <div class="d-flex justify-content-end align-items-center mb-3">
        ...
        <form action="{{ route('beneficiaryNotification.update') }}" method="POST">
            @csrf
    ...
    {{-- Tabs Navigation --}}
   <ul class="nav nav-tabs mb-4 flex-nowrap overflow-x-auto text-nowrap pb-1">
    <li class="nav-item flex-shrink-0">
        <a class="nav-link {{ $tab == 'unread' ? 'active fw-bold' : '' }}"
            href="{{ route('beneficiary.notification', ['tab' => 'unread']) }}">
            Unread
        </a>
    </li>
    <li class="nav-item flex-shrink-0">
        <a class="nav-link {{ $tab == 'read' ? 'active fw-bold' : '' }}"
            href="{{ route('beneficiary.notification', ['tab' => 'read']) }}">
            Read History
        </a>
    </li>
    <li class="nav-item flex-shrink-0">
        <a class="nav-link {{ $tab == 'all' ? 'active fw-bold' : '' }}"
            href="{{ route('beneficiary.notification', ['tab' => 'all']) }}">
            All
        </a>
    </li>
</ul>
